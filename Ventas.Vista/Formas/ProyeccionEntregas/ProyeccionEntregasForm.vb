...
Public Class ProyeccionEntregasForm
    ...
    Private Sub ProyeccionEntregasForm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        ...
        If confirmar.ShowDialog() = Windows.Forms.DialogResult.OK Then
            ...
            Try
                ...
                objBU.LimpiarSesionUsuarioCerrarPantalla(dtSesiones.Rows(0).Item("SesionActiva").ToString)
    ...
    Private Sub PermisosPerfil()
        ...
        If Framework.Negocios.PermisosUsuarioBU.ConsultarPermiso("VENT_OT_PROY", "VENT_PROYECCION_JEFATURA") Then
            TipoPerfil = 2
    ...
    Private Sub btnMostrar_Click(sender As Object, e As EventArgs) Handles btnMostrar.Click
        Dim objBU As New Negocios.ProyeccionEntregasBU
        Dim dtSesiones As New DataTable
        Dim dtPOriginales As New DataTable
        Dim confirmar As New Tools.ConfirmarForm
    ...
    Private Sub CargarInformacion(ByVal sesionAnterior As Integer, ByVal limpiarSesion As Integer)
        ...
        Dim dtInformacionPedidos As DataTable

        Dim FilasPedidoIds As IEnumerable(Of UltraGridRow)
        ...
        Try
            If limpiarSesion = 1 Or sesionAnterior = 0 Then
                ...
                If CantidadFiltrosSeleccionados = 1 Then
                    objBU.GeneracionDatosProyeccionEntregas(colaboradorProyectando, usuarioIdProyectando, usuarioNombreProyectando, sesionAnterior, filtros)
                Else
                ...
                End If
            Else
                objBU.GeneracionDatosProyeccionEntregasCargarSesionAnterior(usuarioIdProyectando, usuarioNombreProyectando, sesionAnterior)
            End If
        Catch ex As Exception
...
#End Region
    ...
    Private Sub CargarInformacionNiveles(ByVal Nivel As Integer, Optional ByVal PedidoSAY As String = "0", Optional ByVal PedidoSICY As String = "0", Optional ByVal Lote As String = "0", Optional ByVal Partida As String = "0", Optional ByVal Año As String = "0", Optional ByVal Nave As String = "0", Optional ByVal Atado As String = "")
        ...
        Select Case Nivel
            Case 1
                DtInformacion = objBU.ConsultaProyeccionPorNivel_Pedido(consultaNivelPedido)
                DtInformacion_Partidas = objBU.ConsultaProyeccionPorNivel_PartidaPedido(consultaNivelPedido)
            Case 2
                DtInformacion = objBU.ConsultaProyeccionPorNivel_Partida(consultaNivelPedido)
            Case 3
                DtInformacion = objBU.ConsultaProyeccionPorNivel_Lote(consultaNivelPedido)
            Case 4
                DtInformacion = objBU.ConsultaProyeccionPorNivel_Atado(consultaNivelPedido)
            Case 5
        ...
        If DtInformacion.Rows.Count > 0 Then
            ...
            If sesionAnterior > 0 And PrimeraConsulta = 1 Then
                dtInformacionOcupada = objBU.ComprobarDatosOcupados(sesionAnterior, filtros)
    ...
    Private Sub GridDiseño(ByVal grid As UltraGrid, ByVal Nivel As Integer)
        ...
        If Nivel = 1 Then
            grid.DisplayLayout.Bands(0).Columns("PedidosCompletos").Hidden = True
        End If
        ...
        If Nivel = 1 Or Nivel = 2 Or Nivel = 3 Then
            For Each Fila As UltraGridRow In grid.Rows
                ...
                If Nivel = 2 Then
                    ...
                    If Fila.Cells.Exists("PedidosCompletos") = True Then
                        grid.DisplayLayout.Bands(0).Columns("PedidosCompletos").Hidden = True
                    End If
                    ...
                    If Fila.Cells("LC").Value = "SI" Then
                        ...
                        'If Fila.Cells.Exists("PedidoCompleto") = True Then
                        '    If Fila.Cells("PedidoCompleto").Value = "1" Then
                        '        Fila.Appearance.BackColor = Color.MediumVioletRed
                        '    End If
                        'End If
                    ...
                    Else
                        ...
                        Else
                            ...
                            If Fila.Cells("ClienteSAYID").Value = 1198 Then 'E-COMMERCE
                                If Fila.Cells("COT").Value > 0 And Fila.Cells("COT").Value > Fila.Cells("COT P").Value Then
                                    Fila.Appearance.BackColor = Color.LightGray
                                End If
                            Else
        ...
        If Nivel = 1 Then
            ...
            'grid.DisplayLayout.Bands(0).Columns("PedidosCompletos").Width = 90
    ...
    Private Sub SeleccionarToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SeleccionarToolStripMenuItem.Click
        ...
        Dim filaSeleccionada As IEnumerable(Of UltraGridRow)

        If gridSeleccionadoClicDerecho.Name = "grdNivelPedido" Then
            NIVEL = 1
        ElseIf gridSeleccionadoClicDerecho.Name = "grdNivelPartida" Then
            NIVEL = 2
        ElseIf gridSeleccionadoClicDerecho.Name = "grdNivelLotes" Then
            NIVEL = 3
        ElseIf gridSeleccionadoClicDerecho.Name = "grdNivelAtados" Then
            NIVEL = 4
        ElseIf gridSeleccionadoClicDerecho.Name = "grdNivelPares" Then
            NIVEL = 5
        End If

        If NIVEL = 1 Then
            filaSeleccionada = gridSeleccionadoClicDerecho.Rows.Where(Function(x) x.Selected = True And (x.Appearance.BackColor = Color.MediumSeaGreen Or (x.Cells("LC").Value = "SI" And (x.Appearance.BackColor = Color.MediumSeaGreen Or x.Appearance.BackColor = Color.Empty)) Or (x.Cells("PC").Value = "SI" And x.Cells("totalPartidasCompletas").Value > 0 And x.Appearance.BackColor = Color.Empty) Or (x.Appearance.BackColor = Color.Empty And x.Cells("PC").Value = "NO" And x.Cells("INV").Value > 0 And x.Cells("ClienteSAYID").Value <> "816") Or (x.Cells("ClienteSAYID").Value = "816" And (x.Cells("INV").Value + x.Cells("PRC").Value = x.Cells("Por Entregar").Value))))
        ElseIf NIVEL = 2 Or NIVEL = 3 Then
            filaSeleccionada = gridSeleccionadoClicDerecho.Rows.Where(Function(x) x.Selected = True And (x.Appearance.BackColor = Color.MediumSeaGreen Or (x.Appearance.BackColor = Color.Empty And x.Cells("PC").Value = "NO" And x.Cells("INV").Value > 0 And x.Cells("ClienteSAYID").Value <> "816")))
        Else
            filaSeleccionada = gridSeleccionadoClicDerecho.Rows.Where(Function(x) x.Selected = True And (x.Appearance.BackColor = Color.MediumSeaGreen Or (x.Appearance.BackColor = Color.Empty And x.Cells("INV").Value > 0 And x.Cells("ClienteSAYID").Value <> "816")))
        End If

        For Each fila As UltraGridRow In filaSeleccionada
            fila.Cells("columnaModificada").Value = 1
        Next

        If NIVEL = 1 Then
            If filaSeleccionada.Count() > 0 Then
                MarcarProyeccion(gridSeleccionadoClicDerecho, NIVEL, False, NIVEL)
                ActualizarGrid(NIVEL)
                MarcarProyeccionLotesCompletos(1, False)
            End If
        Else
            If filaSeleccionada.Count() > 0 Then
    ...
    Private Sub QuitarSeleccionToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles QuitarSeleccionToolStripMenuItem.Click
        ...
        'Dim filaSeleccionada = gridSeleccionadoClicDerecho.Rows.Where(Function(x) x.Selected = True And (x.Appearance.BackColor = Color.MediumSeaGreen Or (x.Appearance.BackColor = Color.Empty And x.Cells("PC").Value = "NO") And x.Cells("INV").Value > 0))
        ...
        If NIVEL = 1 Then
            If filaSeleccionada.Count() > 0 Then
                MarcarProyeccion(gridSeleccionadoClicDerecho, NIVEL, True, NIVEL)
                ActualizarGrid(NIVEL)
                MarcarProyeccionLotesCompletos(1, True)
            End If
        Else
    ...
    Private Sub FiltrarPedidosCompletos()
        ...
        '    If row.Cells("PedidosCompletos").Value = 1 Then
        '        row.Appearance.BackColor = pnlCompleto.BackColor
        '    End If
        'Next
    ...
    Private Sub CargarSeguimientoPares(ByVal Tipo As Integer)
        Try
            Cursor = Cursors.WaitCursor
            Dim Form As New SeguimientoParesProyeccionForm()
