...
Public Class ProyeccionProduccionForm
    ...
    Private Sub consultarDatos()
        Dim datatable = objInstancia.ConsultarPProyyeccion(cbanio.Text)
        data = datatable
        grdPProduccion.DataSource = Nothing
        vwPProduccion.Columns.Clear()
        grdPProduccion.DataSource = datatable
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If vwPProduccion.RowCount > 0 Then
        ...
        Try
            With fbdUbicacionArchivo
                ...
                If ret = Windows.Forms.DialogResult.OK Then
                    If vwPProduccion.GroupCount > 0 Then
                        grdPProduccion.ExportToXlsx(.SelectedPath + "\Calendario_Semanal_" + fecha_hora + ".xlsx", exportOptions)
                    Else
                        grdPProduccion.ExportToXlsx(.SelectedPath + "\Calendario_Semanal_" + fecha_hora + ".xlsx", exportOptions)
                    End If
    ...
    Sub importarExcel(ByVal tabla As GridControl)
        ...
        If myFileDialog.FileName.ToString <> "" Then
            ...
            Try
                da = New OleDbDataAdapter("SELECT * FROM  [ProduccionSemanal$]", conn)
                conn.Open()
                da.Fill(ds, "MyData")
                dt = ds.Tables("MyData")
    ...
    Private Sub DiseñoGridd()
        ...
        DiseñoGrid.DiseñoBaseGrid(vwPProduccion)
        vwPProduccion.OptionsView.ColumnAutoWidth = False
        vwPProduccion.IndicatorWidth = 35
        DiseñoGrid.EstiloColumna(vwPProduccion, "SEMANA", "SEMANA", True, DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains, False, False, 100, False, Nothing, "")
        DiseñoGrid.EstiloColumna(vwPProduccion, "AÑO", "AÑO", True, DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains, False, False, 100, False, Nothing, "")
        DiseñoGrid.EstiloColumna(vwPProduccion, "PROY DE PRODUCCION", "PROY DE PRODUCCION", True, DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains, False, False, 100, False, Nothing, "")

        For Each col As DevExpress.XtraGrid.Columns.GridColumn In vwPProduccion.Columns
            If (col.FieldName = "SEMANA" Or col.FieldName = "AÑO") Then
            ...
            End If
        Next
        lblTotalParesProceso.Text = String.Format("{0:N0}", vwPProduccion.DataRowCount)
    End Sub
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            show_message("Advertencia", "No puedes modificar datos de años anteriores")
        End If

        Cursor = Cursors.Default
    End Sub
    'Estadistica de ventas'
    Private Sub viewInventario_CustomDrawRowIndicator(sender As Object, e As RowIndicatorCustomDrawEventArgs) Handles vwPProduccion.CustomDrawRowIndicator
    ...
    Private Sub insertarDiasActualizados()
        ...
        Try
            If vwPProduccion.RowCount > 0 Then
                ...
                myDatatView = CType(grdPProduccion.DataSource, DataTable)
                Dim result = formaCadena(myDatatView)
                If result <> "EI" Then
                    objInstancia.AgregarPProduccion(result, cbanio.Text)
                    consultarDatos()
                ...
                End If
            Else
                show_message("Advertencia", "No hay datos que guardar")
    ...
    Private Function formaCadena(ByVal tabla As DataTable) As String
        ...
        For index As Integer = 0 To numeroFilas - 2 Step 1

            Dim Semana = tabla.Rows(index).Item("SEMANA").ToString() & "-A"
            Dim Anio = tabla.Rows(index).Item("AÑO").ToString() & "-B"
            Dim Pares = tabla.Rows(index).Item("PROY DE PRODUCCION").ToString() & "-C"

            If Anio = cbanio.Text & "-B" Then
                If Semana <> "-A" And Anio <> "-B" And Pares <> "-C" Then
                    If cadena = String.Empty Then
                        cadena = cadena & Semana & "," & Anio & "," & Pares
                    Else
                        cadena = cadena & "#" & Semana & "," & Anio & "," & Pares
                    End If
                End If
    ...
    Private Sub UpdateGridDataSource()
        clone = data
        grdPProduccion.BeginInvoke(New MethodInvoker(AddressOf AnonymousMethod1))
        data = clone
    End Sub
    Private Sub AnonymousMethod1()
        grdPProduccion.DataSource = clone
    End Sub
    ...
    Private Sub vwSemanas_customCell(sender As Object, e As RowCellCustomDrawEventArgs) Handles vwPProduccion.CustomDrawCell
