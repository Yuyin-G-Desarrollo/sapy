...
Public Class ModSalarioPDFAcuseForm
                ...
                If objMensajeConf.ShowDialog = Windows.Forms.DialogResult.OK Then
                    If subirArchivo(txtArchivo.Text) Then
                        ...
                        Else
                            ...
                            Try
                                credinfonavit = obtenerCreditoInfonavit(solicitudId)

                                If Not credinfonavit Is Nothing Then
                                    resultado = objBU.rechazarIdseSolicitud(solicitudId, directorio, IO.Path.GetFileName(txtArchivo.Text), motivoRechazo, credinfonavit.CIDescuentosemanal.ToString("N2"), credinfonavit.CIRetenciondiaria.ToString("N2"))
                        ...
                        If resultado = "EXITO" Then
                            ...
                            'If imssSemanal <> 0 Then
                            '    modSalario.MDDescuentoImssSem = imssSemanal
                            'End If
    ...
    Private Sub enviar_correo(ByVal modSalario As Entidades.ModificacionSalario, ByVal tipoCorreo As String)
        ...
        If tipoCorreo = "Aceptado" Then
            ...
            asunto = "Confirmación de Modificación de Salario IMSS de " & modSalario.MDColaborador
        ElseIf tipoCorreo = "Rechazado" Then
            mensaje = cuerpoCorreoRechazado(modSalario)
            asunto = "Rechazo IDSE de Modificación de Salario IMSS de " & modSalario.MDColaborador
        End If

        dtDatosCorreo = objBU.consultaDatosCorreo(modSalario.MDID)
        If Not dtDatosCorreo Is Nothing AndAlso dtDatosCorreo.Rows.Count > 0 Then
    ...
    Private Function cuerpoCorreoAceptado(ByVal modSalario As Entidades.ModificacionSalario) As String
        Dim mensaje As String = String.Empty
        Dim valorISR As Double = 0
        Dim objMensajeAdv As New Tools.AdvertenciaForm
        Dim encabezadoISR As String = String.Empty

        valorISR = Math.Abs(modSalario.MDDescuentoISRSem)
        If modSalario.MDDescuentoISRSem > 0 Then
        ...
        mensaje &= "<tbody>" &
                "<tr>" &
                "<td>" & modSalario.MDNSS & "</td>" &
                "<td>" & modSalario.MDAPaterno & "</td>" &
                "<td>" & modSalario.MDAMaterno & "</td>" &
                "<td>" & modSalario.MDNombre & "</td>" &
                "<td>" & modSalario.MDFechaMovimiento & "</td>" &
                "<td style='text-align:right;'>" & Math.Round(modSalario.MDDescuentoImssSem).ToString("C2") & "</td>" &
                "<td style='text-align:right;'>" & Math.Round(valorISR).ToString("C2") & "</td>" &
                "<td style='text-align:right;'>" & Math.Round(modSalario.MDDescuentoInfonavitNuevo).ToString("C2") & "</td>" &
                "</tr>"
        mensaje &= "</tbody>"
        mensaje &= "</table>"
    ...
    Private Function cuerpoCorreoRechazado(ByVal modSalario As Entidades.ModificacionSalario) As String
        Dim mensaje As String = String.Empty
