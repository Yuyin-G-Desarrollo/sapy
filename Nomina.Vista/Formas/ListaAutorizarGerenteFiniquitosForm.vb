...
Public Class ListaAutorizarGerenteFiniquitosForm
    ...
    Dim EstatusInicial As String = "B"
    Private Sub ListaAutorizarGerenteFiniquitos_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        grdFiltroSolicitudes.Columns.Add("TotalEntregar", "Total Entregar") '10
        ...
        grdFiltroSolicitudes.Columns.Add("FechaSolicitud", "Fecha Solicitud") '12
        ...
        grdFiltroSolicitudes.Columns.Add("FechaBaja", "Fecha Baja") '13
        ...
        grdFiltroSolicitudes.Columns.Add("FechaAutorizacion", "Fecha Autorización") '14
    ...
    Public Sub AgregarFila(ByVal ListaFiniquitos As Entidades.Finiquitos)
        ...
        If ListaFiniquitos.PEstado = "B" Then
    ...
    Public Sub AutorizarSolicitudes()
        ...
        For Each row As DataGridViewRow In grdFiltroSolicitudes.Rows 'Recorro todo el grid fila por fila
            ...
            If valorCheck <> 0 Then
                ...
                seleccion = CInt(grdFiltroSolicitudes.Rows(sentinela).Cells(20).Value)
                objBUFiniquito.AceptarSolicitudGerente(seleccion, Entidades.SesionUsuario.UsuarioSesion.PUserUsuarioid)
                CountAcept += 1
            End If
            sentinela += 1
        Next
    ...
    'Private Sub grdFiltroSolicitudes_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
    ...
    '    Dim objBUFiniquito As New Negocios.FiniquitosBU
    '    Dim sentinela As Integer = 0
    '    Dim Actualizar As New Integer
    '    Dim Utilidades, Gratificacion, TotalEntregar As New Double
    '    Dim TotalEntregarCalculo As New Double
    '    Actualizar = CInt(grdFiltroSolicitudes.Rows(Columna).Cells(19).Value)
    '    ValorCheck = CInt(grdFiltroSolicitudes.Rows(Columna).Cells(0).Value)
    '    If grdFiltroSolicitudes.Rows(Columna).Cells(8).Value > 0 Then
    '        Utilidades = CDbl(grdFiltroSolicitudes.Rows(Columna).Cells(8).Value)

    '    Else
    '        Utilidades = 0
    '    End If

    '    If grdFiltroSolicitudes.Rows(Columna).Cells(9).Value > 0 Then
    '        Gratificacion = CDbl(grdFiltroSolicitudes.Rows(Columna).Cells(9).Value)
    ...
    '    Dim ConversionAguinaldo As String = grdFiltroSolicitudes.Rows(Columna).Cells(3).Value.ToString

    '    If grdFiltroSolicitudes.Rows(Columna).Cells(4).Value > 0 Then
    '        Aguinaldo = (grdFiltroSolicitudes.Rows(Columna).Cells(3).Value.ToString.Replace("$", ""))
    '        Aguinaldo = Aguinaldo
    '    Else
    '        Aguinaldo = 0
    '    End If
    ...
    '    If grdFiltroSolicitudes.Rows(Columna).Cells(4).Value > 0 Then
    '        Vacaciones = grdFiltroSolicitudes.Rows(Columna).Cells(4).Value
    ...
    '    Dim Finiquito As Double
    '    If grdFiltroSolicitudes.Rows(Columna).Cells(5).Value > 0 Then
    '        Finiquito = grdFiltroSolicitudes.Rows(Columna).Cells(5).Value
    '        Finiquito = Finiquito
    '    Else
    '        Finiquito = 0
    '    End If
    '    Dim Ahorro As Double
    '    If grdFiltroSolicitudes.Rows(Columna).Cells(6).Value > 0 Then
    '        Ahorro = grdFiltroSolicitudes.Rows(Columna).Cells(6).Value
    '        Ahorro = Ahorro
    ...
    '    If grdFiltroSolicitudes.Rows(Columna).Cells(7).Value > 0 Then
    '        Prestamo = grdFiltroSolicitudes.Rows(Columna).Cells(7).Value
    '        Prestamo = Ahorro
    ...
    '    TotalEntregar = (Aguinaldo + Vacaciones + Finiquito + Ahorro + Utilidades + Gratificacion) - Prestamo
    '    objBUFiniquito.ModificarSolicitud(Actualizar, Utilidades, Gratificacion, TotalEntregar, )
    '    grdFiltroSolicitudes.Rows(Columna).Cells(10).Value = TotalEntregar

    '    ' grdFiltroSolicitudes.Rows.Clear()
    '    'llenartabla()
    '    'grdFiltroSolicitudes.Rows(Columna).Cells(0).Value = ((ValorCheck) * (-1))
    '    'Pintartabla()
    'End Sub
    ...
    Private Sub btnFiltrarSolicitud_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Else
            ...
            Dim advertencia As New AdvertenciaForm
            advertencia.mensaje = "Debe seleccionar una nave para realizar la búsqueda"
            advertencia.ShowDialog()
        End If
    ...
    Private Sub btnAbajo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        grdFiltroSolicitudes.Height = 110
    End Sub

    Private Sub btnAutorizar_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAutorizar.Click
    ...
    Private Sub lblNave_Click(sender As Object, e As EventArgs) Handles lblNave.Click
    ...
    Private Sub lblNombreEmpleado_Click(sender As Object, e As EventArgs) Handles lblNombreEmpleado.Click
