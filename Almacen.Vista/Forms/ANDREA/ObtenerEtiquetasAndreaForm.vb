...
Public Class ObtenerEtiquetasAndreaForm

    Private Sub ObtenerEtiquetasAndreaForm_Load(sender As Object, e As EventArgs) Handles Me.Load
        Me.Location = New Point(0, 0)
        lblCantidadAtadosH.Text = "0"
        lblCantidadAtadosNormales.Text = "0"
    End Sub
    ...
    Private Sub btnBuscarArchivo_Click(sender As Object, e As EventArgs) Handles btnBuscarArchivo.Click
        ...
        Dim listadoCodigosAndrea As New List(Of String)

        Dim ListaCodigosAndreaNormales As New List(Of String)
        Dim ListaAtadosNormales As New List(Of String)
        Dim ListaAtadosH As New List(Of String)
        ...
        Try
            If BuscarArchivo.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
                ...
                While (fileReader.EndOfStream) = False
                    ...
                    If Atado = String.Empty Then
                        Atado = stringReader
                        listadoCodigosAndrea.Clear()
                    Else
                        If stringReader.Trim().Length <= 11 Then
                            Atado = stringReader
                            If listadoCodigosAndrea.Count > 0 Then
                                If listadoCodigosAndrea.Distinct().Count = 1 Then
                                    ListaAtadosNormales.Add(Atado)
                                    For Each Elemento As String In listadoCodigosAndrea
                                        ListaCodigosAndreaNormales.Add(Elemento)
                                    Next
                                Else
                                    ListaAtadosH.Add(Atado)
                                End If
                            End If
                            listadoCodigosAndrea.Clear()
                        Else
                            listadoCodigosAndrea.Add(stringReader)
                        End If
                ...
                DibujarPantalla(ListaCodigosAndreaNormales, ListaAtadosNormales, ListaAtadosH)                
    ...
    Private Sub DibujarPantalla(ByVal ListaCodigosAndreaNormales As List(Of String), ByVal ListaAtadosNormales As List(Of String), ByVal ListaAtadosH As List(Of String))
        Dim DtInformacionEtiquetasAndrea As New DataTable

        DtInformacionEtiquetasAndrea.Columns.Add("CodigoAndrea", GetType(String))
        DtInformacionEtiquetasAndrea.Columns.Add("NumeroEtiquetas", GetType(Integer))

        Dim grupo = ListaCodigosAndreaNormales.GroupBy(Function(x) x)

        For Each grp In grupo
            DtInformacionEtiquetasAndrea.Rows.Add(grp(0), grp.Count / 5)
        Next
        ...
        grdEtiquetasAndrea.DataSource = DtInformacionEtiquetasAndrea

        lblCantidadAtadosNormales.Text = ListaAtadosNormales.Count().ToString()
        lblCantidadAtadosH.Text = ListaAtadosH.Count().ToString()
    End Sub
    ...
    Private Sub btnExportar_Click(sender As Object, e As EventArgs) Handles btnExportar.Click
        exportarExcel(grdEtiquetasAndrea)
    End Sub
    ...
    Private Sub DiseñoGrid(grid As Infragistics.Win.UltraWinGrid.UltraGrid)
        ...
        AgregarColumna(grid, "CodigoAndrea", "Código Andrea", False, True, Nothing, 240, False, False, HAlign.Left)
        AgregarColumna(grid, "NumeroEtiquetas", "Cantidad de Etiquetas", False, True, Nothing, 100, True, False, HAlign.Right, , , , SummaryType.Sum)
